<script lang="ts">
	...
	let pendingUpdates = new SvelteMap<string, number>();
	let updateTimers = new SvelteMap<string, ReturnType<typeof setTimeout>>();
	let savingItems = new SvelteSet<string>();
	...
	async function handleRemoveFromCart(itemId: string) {
		...
		try {
			...
			if (success) {
				toastMessage = message || 'Item removed';
				showSuccess = true;
				await invalidateAll();
				setTimeout(() => (showSuccess = false), 3000);
			} else {
				toastMessage = message || 'Failed to remove item';
				showError = true;
				setTimeout(() => (showError = false), 3000);
			}
			await invalidateAll();
		...
		updateTimers.set(
			...
			setTimeout(async () => {
				...
				if (q !== undefined) {
					...
					savingItems = new SvelteSet(savingItems);
...
<div class="min-h-screen w-full bg-rose-50">
	...
	{:else}
		{#each getGroupedByStore() as group (group.storeId)}
				...
				<div class="bg-pp-white mx-2 rounded-lg p-3 shadow">
					...
					{#each group.items as item (item.id)}
						<div class="mb-3 flex items-center border-b pb-3 last:border-b-0">
							...
							<div class="ml-3 flex flex-1 flex-col">
								...
								<div class="mt-2 flex items-center gap-2">
									...
									{#if savingItems.has(item.id)}
										<div
											class="border-pp-pink h-4 w-4 animate-spin rounded-full border-2 border-t-transparent"
										></div>
									{:else}
							...
							<button class="text-pp-pink text-sm" onclick={() => handleRemoveFromCart(item.id)}>
								Remove
							</button>
						</div>
					{/each}
